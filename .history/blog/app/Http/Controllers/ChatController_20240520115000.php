...
class ChatController extends Controller
{
    ...
    public function index($receiver_id)
    {
        ...
        if(Auth::user()->id == $receiver_id){
            session()->flash('error', '不能與自己聊天');
            
            // 將用戶重定向回上一個URL
            return redirect()->back();
        }
        $messages = Messagenew::where(function($query) {
            $userId = Auth::user()->id;
    ...
    public function store($receiver_id)
    {
        ...
        if($message->user_id == Auth::user()->id && $message->receiver_id != Auth::user()->id)
            $user_name = User::select('username')->where('id',$message->receiver_id)->first();
        else
            $user_name = User::select('username')->where('id',$message->user_id)->first();
        broadcast(new msg($message->user_id, $message->receiver_id, $message->message,$user_name));
        ...
        if($message->user_id == Auth::user()->id && $message->receiver_id != Auth::user()->id){
            ...
            ,Carbon::parse($message->created_at)->diffForHumans(),$user_name));
            broadcast(new msg_outside($message->id,$message->user_id, $message->receiver_id, $message->message
            ,Carbon::parse($message->created_at)->diffForHumans(),$user_name));
        } elseif($message->receiver_id == Auth::user()->id && $message->user_id != Auth::user()->id) {
            //dd(2);
