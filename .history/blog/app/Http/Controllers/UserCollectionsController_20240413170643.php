...
use Carbon\Carbon;
use App\Models\User_collections_artwork;
use App\Models\User_collections_mjoin;
use App\Models\User_collections_shop;
...
class UserCollectionsController extends Controller
{
    public function collection_artwork($article_id){
        ...
        if($user) {
            $user_id = $user->id;
            //有無找到數據 
            $ucas = User_collections_artwork::where('article_id',$article_id)
                                                                ->where('user_id',$user_id)
                                                                ->first();
            if(!$ucas){
                $ucas = new User_collections_artwork();
    ...
    public function collection_mjoin($article_id){
        ...
        if($user) {
            $user_id = $user->id;
            //有無找到數據 
            $ucas = User_collections_mjoin::where('article_id',$article_id)
                                                                ->where('user_id',$user_id)
                                                                ->first();
            if(!$ucas){
                $ucas = new User_collections_mjoin();
                $ucas->user_id = $user_id;
                $ucas->article_id = $article_id;
                $ucas->status = '1';
                $ucas->save();
    ...
    public function collection_shop($article_id){
        ...
        if($user) {
            $user_id = $user->id;
            //有無找到數據 
            $ucas = User_collections_shop::where('article_id',$article_id)
                                                                ->where('user_id',$user_id)
                                                                ->first();
            if(!$ucas){
                $ucas = new User_collections_shop();
                $ucas->user_id = $user_id;
                $ucas->article_id = $article_id;
                $ucas->status = '1';
                $ucas->save();
