    ...
    public function mjoin_post_ungood($mjoinId)
    {
        ...
        if(Auth::check()){
            $goodfind = UserGood::where('reply_id',$mjoinId)
                ...
                $goodfind->many = $goodfind->status == 'false' ? '1' : '0'; // 根據當前的狀態更新按讚數
                $goodfind->status = $goodfind->status == 'false' ? 'true' : 'false'; // 切換按讚狀態
                $goodfind->update();                
    ...
    public function mjoin_post_good($mjoinId)
    {
        ...
        if(Auth::check()){
            ...
            if(!$goodfind){
                ...
                $good->many = $good->status == 'false' ? '1' : '0'; // 根據當前的狀態更新按讚數
                $good->status = $good->status == 'false' ? 'true' : 'false'; // 切換按讚狀態
                $good->update();
