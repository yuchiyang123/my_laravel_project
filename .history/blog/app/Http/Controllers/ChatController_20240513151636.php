...
class ChatController extends Controller
{
    ...
    ) AS t1 ON messages.receiver_id = t1.receiver_id AND messages.id = t1.max_id
    RIGHT JOIN (
        SELECT user_id, MAX(id) AS max_id
        FROM messages
        WHERE receiver_id = '.Auth::user()->id.'
        GROUP BY user_id
    ) AS t2 ON messages.user_id = t2.user_id AND messages.id = t2.max_id
    WHERE t1.receiver_id IS NULL OR t2.user_id IS NULL
