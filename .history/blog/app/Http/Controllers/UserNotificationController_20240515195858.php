    ...
    public function allnotify(){
        ...
        if(!Auth::check()){
            return redirect('/login');
                ...
                $query5 = DB::table('join_mjoin')
                                ...
                                ->whereNotIn('join_mjoin.status', ['pass', 'reject','complete']) // 排除状态为 pass 或 reject 的记录
                                ->where('mjoin.posted_by_u', Auth::user()->username)
        ...
        foreach ($notifications as $notification) {
            ...
            if ($notification->source === 'reply_artwork') {
                ...
                $url = '/arkwork_solo/solo/' . $notification->artwork_id;
            } elseif ($notification->source === 'great_arkwork') {
                $imageDataUri = 'data:' . $notification->profileImage_type  . ';base64,' . base64_encode( $notification->profileImage );
                // 如果通知来源于 mjoin 回复的表
                $content = '您收到了一則關於您的作品的讚';
                $url = '/arkwork_solo/solo/' . $notification->artwork_id;
            } elseif ($notification->source === 'reply') {
                $imageDataUri = 'data:' . $notification->profileImage_type  . ';base64,' . base64_encode( $notification->profileImage );
                // 如果通知来源于 mjoin 回复的表
                $mjoin = UserPostMjoin::where('id', $notification->reply_id)->first();
