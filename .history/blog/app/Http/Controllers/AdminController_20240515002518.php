...
class AdminController extends Controller
{
    ...
    public function user_artwork_delete($arkId,Request $request)
    {
        ...
        if($request->input('msg')){
            ...
            $notify->save();
            if (Route::currentRouteName() == 'user_join') {
                return redirect()->route('user_join')->with('success', '刪除成功');
            } else if (Route::currentRouteName() == 'user_cms') {
                return redirect()->route('user_cms')->with('success', '刪除成功');
            }
        }
    ...
    //CMS打工貼文回復
    public function user_artwork_delete_back($shopId)
    {
