...
class AdminController extends Controller
{
    ...
    public function user_join_status($userId){
        ...
        $mjoins = UserPostMjoin::where('posted_by_u',$user->username)->get();
        ...
        // 將所有處理好的資料傳遞到視圖中
        $user = User::where('id',$userId)->first();
        $shops = UserPostMjoin::where('posted_by_u',$user->username)->get();
        ...
        foreach ($shops as $shop) {
            // 查詢這篇文章的加入者
            if (isset($shop_joiners[$shop->id]) && !$shop_joiners[$shop->id]->isEmpty()) {
            $shop_joiners[$mjoin->id] = Shop_join::where('article_id', $shop->id)->get();
        ...
        return view('user_join_status',compact('joiners','mjoins','shop_joiners','shops'));
    }
