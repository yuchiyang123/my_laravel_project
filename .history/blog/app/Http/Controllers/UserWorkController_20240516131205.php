...
class UserWorkController extends Controller
{
    public function work_post_form()
    {
        ...
        $shop_1 = UserPostWork::where('posted_by_u',Auth::user()->username)->get();
        ...
        foreach($shop_1 as $shop_2){
            $shop_join = Shop_join::where('article_id', $shop_2->id)->where('user_id', Auth::user()->id)->first();
            if($shop_2->status != 'pending'){
                if($shop_join->status == 'pending'){
