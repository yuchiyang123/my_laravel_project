...
class ChatController extends Controller
{
    ...
    public function index($receiver_id)
    {
        ...
        $messages = Messagenew::whereIn('id', function ($query) {
            $query->selectRaw('MAX(id)')
                ->from('messages as m1')
                ->where(function ($subQuery) {
                    $subQuery->where('m1.user_id', Auth::user()->id)
                        ->orWhere('m1.receiver_id', Auth::user()->id);
                })
                ->groupBy('m1.user_id', 'm1.receiver_id')
                ->havingRaw('COUNT(*) > 1');
        })
        ->orderBy('id', 'desc')
        ->get();
