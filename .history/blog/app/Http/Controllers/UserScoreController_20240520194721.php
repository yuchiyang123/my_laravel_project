...
class UserScoreController extends Controller
{
    public function score_form($mjoin)
    {
        ...
        $mjoin_author = UserPostMjoin::where('id', $mjoin)
                                    ->first();
        $userJoinMjoinStatus1 = User_join_mjoin::where('user_id', Auth::user()->id)
                                    ->where('article_id', $mjoin)->first();
                                    
        $userPostMjoinStatus1 = UserPostMjoin::where('id', $mjoin)
        ...
        if(!$userJoinMjoinStatus1 && !$userPostMjoinStatus1){
            return redirect('/front')->with('error', '您未加入此招募');
    ...
    public function shop_score_form($joinId)
    {
        ...
        $ex1 = WorkScore::where('rater_id',Auth::user()->id)->where('article_id',$joinId)->first();
