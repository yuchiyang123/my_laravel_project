...
$messages1 = Message::select('messages.*')
  ->leftJoinSub(
    Message::selectRaw('max(id) as max_id')
      ->whereColumn('receiver_id', 'messages.receiver_id')
      ->groupBy('receiver_id'),
    'max_messages',
    function($join) use ($user){
      $join->on('messages.id', 'max_messages.max_id'); 
    }
  )  
  ->where('messages.user_id', $user->id)
  ->orWhere('messages.receiver_id', $user->id);

$messages2 = Message::select('messages.*')
  ->leftJoinSub(
    Message::selectRaw('max(id) as max_id')  
      ->whereColumn('user_id', 'messages.user_id')
      ->groupBy('user_id'), 
...
$messages = $messages1->union($messages2)->orderBy('messages.id', 'desc')->get();
