    ...
    public function allnotify(){
        ...
        if(!Auth::check()){
            return redirect('/login');
                ...
                $query1 = DB::table('reply_artwork')
                    ->select('reply_artwork.*','reply_artwork.reply_id as artwork_id' ,'user.profileImage', 'user.profileImage_type', DB::raw('NULL as additional_field'), DB::raw('"reply_artwork" as source'))
                ...
                $query5 = DB::table('join_mjoin')
                                ->select('join_mjoin.id as join_id','join_mjoin.*','mjoin.id as mjoin_id','user.profileImage', 'user.profileImage_type', DB::raw('NULL as additional_field'), DB::raw('"notify_join_mjoin" as source'))
                                ->leftJoin('mjoin', 'join_mjoin.article_id', '=', 'mjoin.id')
                                ->leftJoin('user', 'join_mjoin.user_id', '=', 'user.id')
                                ->whereNotIn('join_mjoin.user_id', [$currentUserId]) // 排除当前用户的申请
                                ->whereNotIn('join_mjoin.status', ['pass', 'reject','complete','rejected','end']) // 排除状态为 pass 或 reject 的记录
                                ->where('mjoin.posted_by_u', Auth::user()->username)
                                ->orderBy('join_mjoin.updated_at', 'desc')
                                ->get();
                
                $query6 = DB::table('shop_join')
                                ->select('shop_join.id as shop_join','shop_join.*','shop.posted_by_u', 'user.profileImage', 'user.profileImage_type', DB::raw('NULL as additional_field'), DB::raw('"shop_join" as source'))
                                ...
                                ->whereNotIn('shop_join.status', ['pass', 'reject','complete','rejected','end']) // 排除状态为 pass 或 reject 的记录
        ...
        foreach ($notifications as $notification) {
            ...
            if ($notification->source === 'reply_artwork') {
                ...
                $imageDataUri = 'data:' . $notification->profileImage_type  . ';base64,' . base64_encode( $notification->profileImage );
                // 如果通知来源于 mjoin 回复的表
                $mjoin = UserPostMjoin::where('id', $notification->reply_id)->where('status','<>','del')->first();
                $content = '<strong>'.$notification->name_u .'</strong>'.'回復您的揪團貼文'.'<strong>'.'「'. $mjoin ->title . '」'.'</strong>';
                $time = $notification->created_at;
                $url = '/mjoin_solo/' . $notification->reply_id;
                //$url= '';
                ...
                $url_join = '/join_shop_verify/' . $notification->shop_join;
            ...
            if($notification->source != 'notify_join_mjoin' && $notification->source != 'shop_join'){
                if (!$lastReadTime || $notification->updated_at > $lastReadTime) {
                    ...
                    $htmlContent .= '<span class="fw-bolder">' . $content . '</span>';
                    $htmlContent .= '<br><span class="text-secondary">' . $time . '</span>';
                    $htmlContent .= '</a></li>';
                } else {
                    $htmlContent .= '<li><a class="dropdown-item" href="' . $url . '" id="join_normalnotify">';
                    if($notification->profileImage!= null){
                        $htmlContent .= '<div class="col"><img src="' . $imageDataUri . '" alt="mdo" width="50" height="50" class="rounded-circle">';
                    } else {
                        $htmlContent .= '<img src="https://github.com/mdo.png" alt="mdo" width="50" height="50" class="rounded-circle">';
                    }
                    
                    $htmlContent .= '<span class="">' . $content . '</span>';
                    $htmlContent .= '<br><span class="text-secondary">' . $time . '</span></div>';
                    $htmlContent .= '</a></li>';
                }
