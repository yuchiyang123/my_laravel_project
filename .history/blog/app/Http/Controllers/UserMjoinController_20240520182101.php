...
class UserMjoinController extends Controller
{
    ...
    public function mjoin_post_form()
    {
        ...
        foreach ($check2 as $check3) {
            $shop_1 = UserPostMjoin::where('id', $check3->article_id)->first();
            if($shop_1->status == 'pending') {
                return redirect()->route('front')->with('error', '您已經現在有加入其他揪團，請等待結束後再加入');
            }
        }        
        ...
        /*$mjoins = UserPostMjoin::orderBy('id', 'desc')
                           ...
                           foreach ($mjoins as $mjoin) {
                            ...
                            if(Auth::check()){
                                ...
                                ->whereNotIn('mjoin.status',['del', 'end','complete'])
