...
class IndexController extends Controller
{
    public function index(){
        try{
            if(Auth::check()){
                $session_null = DB::table('sessions')->where('user_id',Null)->orderBy('last_activity','desc')->first();
                dd($session_null);
                if ($session_null) {
                    DB::table('sessions')->where('id',$session_null->id)->where('user_id',null)->first()->delete();
                }
            }
