...
class UserController extends Controller
{
    ...
    //全部收藏
    public function user_collections_all($username)
    {
        $user = User::where('username', $username)->first();
        $user_c = DB::table('user_profile')->where('user_id', $user->id)->first();
        if(empty($user_c)){
            $user_col_new = new UserProfile();
            $user_col_new->user_id = $user->id;
            $user_col_new->save();
        }
        ...
        $userinfo = DB::table('user_profile')
            ...
            $userCollections = DB::table('user_collections_artwork')
                              ->select('user_collections_artwork.*', 'artwork.*')
              ...
              $userCollections_mjoin = DB::table('user_collections_mjoin')
                              ...
                              ->get();
        if(Auth::check()){
            if($username == Auth::user()->username || $userinfo){
                if($username == Auth::user()->username || $userinfo->favorite_articles_visibility=='public')
                    return view('auth.all_collect', compact('user','userinfo','userCollections','userCollections_mjoin','userCollections_shop'));
                
                    return redirect('/user-profile/index/d/'.$username)->with('error','該用戶未開放收藏功能');
    ...
    //全部評語
    public function user_score_all($username)
    {
        $user = User::where('username', $username)->first();
            $userid = $user->id;
        $userinfo = DB::table('user_profile')
                              ...
                              ->where('user.username',$username)
                              ->first();
            $usershopscore = DB::table('shop_score')
                              ->where('shop_score.evaluated_id',$userid)
                              ->get();

            $usermjoinscore = DB::table('mjoin_score')
                              ->where('mjoin_score.evaluated_id',$userid)
                              ->get();
              
              

            return view('auth.all_score', compact('user','userinfo','usershopscore','usermjoinscore'));
    ...
    public function user_data($username)
    {
            ...
            $user = User::where('username', $username)->first();
            if(!$user){
                return redirect()->route('error')->with('error','該用戶不存在');
            }
            $userid = $user->id;
            //資料                                
            ...
            $userCollections = DB::table('user_collections_artwork')
                            ->select('user_collections_artwork.*','user_collections_artwork.status as user_collections_artwork-status', 'artwork.*')
                            ->leftJoin('artwork', 'user_collections_artwork.article_id', '=', 'artwork.id')
                            ->where('user_collections_artwork.user_id',$userid)
                            ->where('user_collections_artwork.status', '<>', 0)
                            ->where('artwork.status', '<>', 'del')
            ...
            //dd($averageScore_shop);               
            //dd($userCollections_mjoin,$userCollections_shop);              
                              
            if($userrecord == null){
                return redirect()->route('error');
            }

            return view('auth.userprofile', compact('userrecord' ,'userarkworks','userinfo','userCollections','userCollections_mjoin','userCollections_shop','averageScore',
            'averageScore_shop','averageTotalScore'));
