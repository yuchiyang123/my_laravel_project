    ...
    public function allnotify(){
        ...
        $n_m_j = DB::table('notify_join_mjoin')
                ...
                $notifications = $notifications->unionAll(function ($query) {
                    $query->select('mjoin_reply.*', 'user.profileImage', 'user.profileImage_type')
                        ->from('mjoin_reply')
                        ->leftJoin('mjoin', 'mjoin_reply.reply_id', '=', 'mjoin.id')
                        ->leftJoin('user', 'mjoin_reply.name_u', '=', 'user.username')
                        ->where('mjoin.status', '<>', 'del')
                        ->where('mjoin_reply.status', '<>', 'del')
                        ->where('mjoin.posted_by_u', Auth::user()->username)
                        ->where('mjoin_reply.name_u', '<>', Auth::user()->username);
                });
                
                $notifications = $notifications->unionAll(function ($query) {
                    $query->select('great.*', 'user.profileImage', 'user.profileImage_type')
                        ->from('great')
                        ->leftJoin('mjoin', 'great.reply_id', '=', 'mjoin.id')
                        ->leftJoin('user', 'great.clickgood_u', '=', 'user.username')
                        ->where('mjoin.status', '<>', 'del')
                ...
                $notifications = $notifications->unionAll(function ($query) {
                    $query->select('great_arkwork.*', 'user.profileImage', 'user.profileImage_type')
                        ->from('great_arkwork')
                        ->leftJoin('artwork', 'great_arkwork.article_id ', '=', 'artwork.id')
                        ->leftJoin('user', 'great_arkwork.user_id', '=', 'user.username')
                        ->where('artwork.status', '<>', 'del')
                ...
                $notifications = $notifications->unionAll(function ($query) {
                    $query->select('notify_join_mjoin.*', 'user.profileImage', 'user.profileImage_type')
        ...
        foreach ($notifications as $notification) {
            ...
            $htmlContent_join .= '<li><a class="dropdown-item" href="' . $url . '" id="join_normalnotify">';
            $htmlContent_join .= '<img src="' . $imageDataUri . '" alt="mdo" width="50" height="50" class="rounded-circle">';
            $htmlContent_join .= '<span class="fw-bold">' . $content . '</span>';
