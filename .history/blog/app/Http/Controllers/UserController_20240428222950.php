...
class UserController extends Controller
{
    ...
    public function user_collections_all($username)
    {
        ...
        if($username == Auth::user()->username || $userinfo){
            if($userinfo->favorite_articles_visibility=='public')
    ...
    public function user_score_all($username)
    {
        ...
        $userinfo = DB::table('user_profile')
            ...
            $usershopscore = DB::table('mjoin_score')
                              ->where('mjoin_score.evaluated_id',$userid)
                              ->get();
              
              
        
                return view('auth.user_score', compact('user','userinfo','userCollections','userCollections_mjoin','userCollections_shop'));
            
            
        } else {
            return redirect('/user-profile/index/d/'.$username)->with('error','評分顯示錯誤');
        }
    }
    //用戶資料
