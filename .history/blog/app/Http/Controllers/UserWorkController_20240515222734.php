    ...
    $articleIds = $shops->pluck('id')->toArray(); // 提取文章 ID
                               ...
                               foreach ($shops as $shop) {
                                ...
                                /*foreach ($shops_join as $shop_join) {
                                    ...
                                    //d($shops_join);*/
                                    $mjoin_members = DB::table('join_shop')
                                ->select('join_shop.*','join_shop.status as join_status','shop.*', 'user.profileImage_type', 'user.profileImage', 'user.username')
                                ->leftJoin('user', 'join_shop.user_id', '=', 'user.id')
                                ->leftJoin('shop', 'join_shop.article_id', '=','shop.id')
                                ->whereNotIn('join_shop.status', ['complete','pending','rejected'])
                                ->where('join_shop.user_id', Auth::user()->id)
                                ->distinct()
                                ->get();
                            
                            $mjoin_article_ids = $mjoin_members->pluck('article_id'); // 获取所有文章的 article_id
                            
                            $mjoin_all_member = DB::table('join_shop')
                                ->whereIn('join_shop.article_id', $mjoin_article_ids)
                                ->get();
