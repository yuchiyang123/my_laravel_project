    ...
    public function edit_profile_youinfo_submit(Request $request)
    {
        ...
        if (Auth::check()) {
            ...
            if ($request->has('social_links')) {
                $userProfile->social_links = $request->input('social_links');
            } else {
                $userProfile->social_links = null;
            }
            
            // 检查第二个社交链接
            if ($request->has('social_links2')) {
                $userProfile->social_links2 = $request->input('social_links2');
            } else {
                $userProfile->social_links2 = null;
            }
            
            // 检查第三个社交链接
            if ($request->has('social_links3')) {
                $userProfile->social_links3 = $request->input('social_links3');
            } else {
                $userProfile->social_links3 = null;
            }
            
            // 检查第四个社交链接
            if ($request->has('social_links4')) {
                $userProfile->social_links4 = $request->input('social_links4');
            } else {
                $userProfile->social_links4 = null;
            }
