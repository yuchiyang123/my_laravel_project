...
class UserScoreController extends Controller
{
    public function score_form($mjoin)
    {
        ...
        $mjoin_author = UserPostMjoin::where('id', $mjoin)
                                    ->where('posted_by_u', Auth::user()->id)
                                    ->first();

        // 计算加入且审核通过的用户数量
        $join_members_count = $join_members->count();
        $mjoin_author_count = if($mjoin_author!=null){
        ...
        // 计算结果相加
        $total = $join_members_count + $mjoin_author_count;
        ...
        return view('user.score_form', compact('mjoin_author', 'join_members', 'is_author'));
    }
}