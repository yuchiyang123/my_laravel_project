...
class UserController extends Controller
{
    ...
    public function __construct()
    {
        $this->firebase = (new Factory)
            ->withServiceAccount(__DIR__.'/path/to/firebase_credentials.json')
            ->create();
    }

    public function sendSmsVerificationCode($phoneNumber, $code)
    {
        // 調用 Firebase SDK 發送短信驗證碼
        $messaging = $this->firebase->getMessaging();
        $message = CloudMessage::withTarget('phone', $phoneNumber)
            ->withNotification([
    ...
    public function sendVerificationCode(Request $request)
    {
        ...
        $auth->sendSmsVerificationCode($phoneNumber, ['code' => $code]);

        session(['verification_code' => $code]);
        // 將驗證碼存入 session 中
        //$request->session()->put('verification_code', $code);
        //dd($request->session->get('verification_code'));

        //return redirect()->route('phone_verification.verify');
        //$verificationCode = $request->session()->get('verification_code');
        //dd(session('verification_code'),$request);
        //return view('verify_phone', compact('verificationCode'));
        //return redirect()->route('front');
        return view('verify_phone_input');
