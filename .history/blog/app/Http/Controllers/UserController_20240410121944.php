...
class UserController extends Controller
{
    ...
    {
        $usernameu = session('user_name');
        $userarkworksolo = UserPostArkwork::where('id',$ark_id)
                                          ->where('status','<>','del')
                                          ->first();
        $userartworkreplys = Artwork_reply::where('reply_id' ,$ark_id)->get();
        //全部按讚人數
        $usergoods = UserGood::where('reply_id',$ark_id)
                             ->get();
        //單獨按讚人數
        $usersologood = UserGood::where('reply_id',$ark_id)
                                ->where('clickgood_u',$usernameu)
                                ->first();
        //全部按讚人數計算
        $usergoodcount = UserGood::where('reply_id',$ark_id)
                                 ->where('many',"1")
                                 ->count();
        return view('auth.arkworksolo' ,compact('userarkworksolo','userartworkreplys','usergoodcount','usergoods','usersologood'));
    }
    //編輯貼文表單邏輯設計
    ...
    public function artwork_del($ark_id)
    {
        ...
        if(Auth::check()){
            ...
            $artwork_del->update();
            return redirect()->route('user_profile_d',['username'=> $artwork_del->posted_by_u]);
        }else {
    ...
    public function artwork_del_a($ark_id)
    {
        ...
        if(Auth::check()){
            $artwork_del->status = "del";
            $artwork_del->update();
            return redirect()->route('arkwork_all',['username'=> $artwork_del->posted_by_u]);
        }else {
            return redirect()->route('login')->with('error','請先登入好嗎');
        }
    }
