...
class UserEditProfileController extends Controller
{
    ...
    public function edit_profile_youinfo_from()
    {
        if(Auth::check()){
            ...
            if(!$user_check_null){
                ...
                $user_cr_null->user_id = $user_data->id;
                $user_cr_null->favorite_articles_visibility = 'private';
                $user_cr_null->save();
            }
    ...
    public function edit_profile_youinfo_submit(Request $request)
    {
        if (Auth::check()) {
            ...
            $userProfile->favorite_articles_visibility = $request->has('favorite_articles_visibility') ? 'public' : 'private';

            $userProfile->self_introduction = $request->has('self_introduction') ? $request->input('self_introduction') : null;
            ...
            return redirect()->route('edit_profile_youinfo_from')->with('success', '修改成功');
        } else {
            return redirect()->route('login')->with('error', '請先登入');
        }
    }
