        ...
        /*$mjoins = UserPostMjoin::orderBy('id', 'desc')
                           ...
                           foreach ($mjoins as $mjoin) {
                            ...
                            /*foreach ($join_mjoin as $shop_join) {
                                ...
                                //dd($join_mjoin);
                $mjoin_member = DB::table('join_mjoin')
                                ->select('join_mjoin.*', 'user.profileImage_type', 'user.profileImage', 'user.username')
                                ->leftJoin('user', 'join_mjoin.user_id', '=', 'user.username')
                                ->leftJoin('mjoin', 'join_mjoin.article_id', '=','mjoin.id')
                                ->where('mjoin.id','join_mjoin.article_id')
                                ->where('mjoin.status', '<>', ['complete','end'])
                                ->distinct()
                                ->get();
                                               

        return view('auth.front', compact('mjoins','checks','mjoin_member'));
    }
