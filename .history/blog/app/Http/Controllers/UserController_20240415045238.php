...
class UserController extends Controller
{
    ...
    public function sendVerificationCode(Request $request, $phone)
    {
        // 验证手机号格式
        ...
        // 替换成您的 Firebase 项目的 Web API Key
        $apiKey = 'AIzaSyDnY37byXfJH6EDSovfolM27bYV_k1r7ko';
    
        // 构建请求的数据
        $data = [
            'phoneNumber' => $phone,
            'recaptchaToken' => '6Ldp87opAAAAALmAnDM6qMIuU1jUNednXo6_l63t',
            'code' => $code,
        ];
    
        // 发送 POST 请求到 Firebase REST API
        $response = Http::withHeaders([
            'Content-Type' => 'application/json',
        ])->post("https://identitytoolkit.googleapis.com/v1/accounts:sendVerificationCode?key=$apiKey", $data);
    
        // 检查是否成功发送验证码
        if ($response->successful()) {
            // 将验证码存储到 session 中，以便后续验证
            $request->session()->put('verification_code', $code);
            return response()->json(['message' => 'Verification code sent', 'phone' => $phone]);
        } else {
            // 处理发送失败的情况
            return response()->json(['error' => 'Failed to send verification code'], $response->status());
        }
    }
