...
class UserWorkController extends Controller
{
    public function work_post_form()
    {
        ...
        $shop_all = UserPostWork::where('posted_by_u',Auth::user()->username)->where('status','pending')->first();
        
        if(Auth::user()->permissions > 2){
            return redirect('/work')->with('error','你不是店家，無法發表工作');
        }
        if($shop_all){
            return redirect('/work')->with('error','你目前有一篇尚未完成的工作，請先完成後再來發表新的工作');
        }
        $shop_1 = Shop_join::where('user_id', Auth::user()->id)->where('status','<>','rejected')->get();
        foreach($shop_1 as $shop_2){
            $shop_3 = UserPostWork::where('id', $shop_2->article_id)->first();
            dd($shop_3);
            if($shop_3->status == 'pending'){
                return redirect('/work')->with('error','您已經目前有一篇工作正在進行中，請先完成後再來發表新的工作');
            }
        }
    ...
    $articleIds = $shops->pluck('id')->toArray(); // 提取文章 ID
                               ...
                               foreach ($shops as $shop) {
                                ...
                                foreach ($shops_join as $shop_join) {
                                    ...
                                    //d($shops_join);
                                    if(Auth::check()){
                                    $mjoin_members = DB::table('shop_join')
                                ->select('shop_join.*','shop_join.status as join_status','shop.*', 'user.profileImage_type', 'user.profileImage', 'user.username')
                                ->leftJoin('user', 'shop_join.user_id', '=', 'user.id')
                                ->leftJoin('shop', 'shop_join.article_id', '=','shop.id')
                                ->whereNotIn('shop_join.status', ['complete','pending','rejected','end'])
                                ->where('shop_join.user_id', Auth::user()->id)
                                ->distinct()
                                ->get();
                            
                            $mjoin_article_ids = $mjoin_members->pluck('article_id'); // 获取所有文章的 article_id
                            
                            $mjoin_all_member = DB::table('shop_join')
                                ->whereIn('shop_join.article_id', $mjoin_article_ids)
                                ->whereNotIn('status', ['complete','pending','rejected','end'])
                                ->get();
                            
                                //你的揪團
                                /*$join_mjoin = DB::table('mjoin')
                                ->select('mjoin.*','mjoin.id as mjoin_id', 'join_mjoin.*', 'mjoin.id as mjoin_id')
                                ...
                                $join_mjoin_you = UserPostWork::where('posted_by_u', Auth::user()->username)
                                                ->whereNotIn('status', ['del', 'end','complete'])
                                                ->first();
                                if(!empty($join_mjoin_you)){
                                    $join_mjoin = DB::table('shop_join')
                                            ->where('article_id',$join_mjoin_you->id)
                                            ->whereNotIn('status', ['complete','pending','rejected'])
                                            ->get();
                                            return view('auth.work', compact('shops','mjoin_members','mjoin_all_member','join_mjoin','join_mjoin_you'));
                                }
                                return view('auth.work', compact('shops','mjoin_members','mjoin_all_member','join_mjoin_you'));
                            }
    ...
    public function shop_reply($shopId)
    {
        ...
        $htmlContent = '<div class="SeeAllMessage">
                        <a href="/shop_solo/'.$shopId.'">查看全部留言</a>
                        </div>';
        ...
        if(Auth::check()){
            if($request->has('editor')) {
                $request->validate([
                    ...
                    'selectwhere' => 'required|string|max:255',
                    'business_registration_number' => 'required|string|max:255',
                    'location' => 'required|string|max:255',
                    'driver_license_requirements' => 'required|string|max:255',
                    'recruitment_period' => 'required|string|max:255',
                    'sex' => 'required|string|max:255',
                    'languages' => 'required|array',
                    'conditions_exp' => 'required|string|max:255',
                    'work_hours' => 'required|string|max:255',
                    'job_description' => 'required|array',
                    'benefits' => 'required|array',
                    'editor' => 'required|string',
                ], [
                    'shop_name.required' => '店名不能為空',
                    'selectwhere.required' => '地區選擇不能為空',
                    'business_registration_number.required' => '商業登記號碼不能為空',
                    'location.required' => '位置不能為空',
                    'driver_license_requirements.required' => '駕照要求不能為空',
                    'recruitment_period.required' => '招聘期限不能為空',
                    'sex.required' => '性別要求不能為空',
                    'languages.required' => '語言要求不能為空',
                    'conditions_exp.required' => '條件與經驗不能為空',
                    'work_hours.required' => '工作時間不能為空',
                    'job_description.required' => '工作描述不能為空',
                    'benefits.required' => '福利不能為空',
                    'editor.required' => '編輯內容不能為空',
                ]);
