    ...
    public function allnotify(){
        ...
        if(!Auth::check()){
            return redirect('/login');
                ...
                $query5 = DB::table('notify_join_mjoin')
                    ->select('notify_join_mjoin.*', 'join_mjoin.id as join_mjoin_id','user.profileImage', 'user.profileImage_type', DB::raw('NULL as additional_field'), DB::raw('"notify_join_mjoin" as source'))
                    ->leftJoin('mjoin', 'notify_join_mjoin.article_id', '=', 'mjoin.id')
                    ->leftJoin('join_mjoin','notify_join_mjoin.user_id', '=', 'join_mjoin.user_id')
                    ->leftJoin('join_mjoin','notify_join_mjoin.article_id', '=', 'join_mjoin.article_id')
        ...
        foreach ($notifications as $notification) {
            ...
            if ($notification->source === 'reply_artwork') {
                ...
                $url_join = '/join_mjoin_verify/' . $notification->join_mjoin_id;
