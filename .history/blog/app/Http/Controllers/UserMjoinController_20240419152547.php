...
class UserMjoinController extends Controller
{
    ...
    public function mjoin_reply($mjoinId)
    {
        ...
        $mjoin_replys = DB::table('Mjoin_reply')
                            ->select('Mjoin_reply.*','user.profileImage_type','user.profileImage','user.profileImage_type')
                            ->leftjoin('user','Mjoin_reply.name_u','=','user.username')
                            ->where('Mjoin_reply.reply_id', $mjoinId)
                            ->get();
        //IF回復為0
