...
class AdminController extends Controller
{
    ...
    {
        $userarkworkedit = UserPostWork::where('id',$artId)->first();
        if(empty($userarkworkedit))
            return redirect()->route('user_cms')->with('success', '找不到資料');
        return view('admin.user_shop_edit',compact('userarkworkedit'));
    }
    ...
    //創作編輯
    public function user_cms_artwork_edit_submit(Request $request)
    {
        if(Auth::check())
        {
