...
class UserController extends Controller
{
    ...
    public function user_register(Request $request)
    {
        ...
        $user->save();
        session(['user_email' => $credentials['email']]);
        session(['user_name' => $credentials['username']]);
        if(Auth::check()){
            $user_data = Auth::user();
            if(UserRecord::where('user_id',$user_data->id)->exists()){
                UserRecord::where('user_id',$user_data->id)
                          ->delete();
            }
            $user_record = new UserRecord();
            $user_record->user_id = $user_data->id;
    ...
    public function user_data($username)
    {
            ...
            $userarkworks = DB::table('artwork')
                              ...
                              ->get();
            if($userrecord == null){
                return redirect()->route('login')->with('error','出錯');
            }
