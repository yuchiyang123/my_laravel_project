...
class IndexController extends Controller
{
    ...
    public function mjoin_search(Request $request)
    {
        ...
        // 處理人數條件
        if ($request->filled('NOpeople')) {
            $query->where('NOpeople', $request->input('NOpeople'));
        } else {
            $query->where('NOpeople', 'LIKE', '%');
        }
        ...
        if ($request->filled('age')) {
            ...
            $query->where('age', 'LIKE', '%');
        }

        $search_all_all = $query->get();
            
            return view('auth.mjoin_search',compact('search_all_all'));
    ...
    public function shop_search(Request $request)
    {
        ...
        $search_all_all = $query->get();
        //dd($search_all_all);
        return view('auth.shop_search',compact('search_all_all'));
        } catch(\Exception $e){
            return view('auth.error');
        }
    }
