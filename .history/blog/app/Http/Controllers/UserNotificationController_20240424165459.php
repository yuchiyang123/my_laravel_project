        ...
        if(!Auth::check()){
            return redirect('/login');
                ...
                $query6 = DB::table('shop_join')
                                ->select('shop_join.*','shop.id', 'user.profileImage', 'user.profileImage_type', DB::raw('NULL as additional_field'), DB::raw('"shop_join" as source'))
                                ->leftJoin('user', 'shop_join.user_id', '=', 'user.id')
                                ->leftJoin('shop', 'shop.user_id', '=', 'user.id')
