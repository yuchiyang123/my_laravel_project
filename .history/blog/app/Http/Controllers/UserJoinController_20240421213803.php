...
class UserJoinController extends Controller
{
    ...
    public function review_mjoin_form($joinId)
    {
        ...
        $review_join_mjoins = User_join_mjoin::where('id',$joinId)->first();
        $review_join_mjoin = User_notify_mjoin::where('id',$joinId)->first();
        $review_join_user = User::where('id',$review_join_mjoins->user_id)->first();
        if(!$review_join_mjoins){
            return redirect()->route('error');
        }
        return view('auth.review_join_mjoin', compact('review_join_mjoin','review_join_user'));
