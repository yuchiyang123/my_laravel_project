        ...
        if(!Auth::check()){
            return redirect('/login');
                ...
                $query5 = DB::table('notify_join_mjoin')
                    ->select('notify_join_mjoin.*','notify_join_mjoin.user_id','user.profileImage', 'user.profileImage_type', DB::raw('NULL as additional_field'), DB::raw('"notify_join_mjoin" as source'))
        ...
        foreach ($notifications as $notification) {
            ...
            if ($notification->source === 'reply_artwork') {
                ...
                if($notification->status == 'pass' && $notification->user_id == Auth::user()->id){
                    ...
                    $url_join = '';
                } elseif ($notification->status =='pending' && $notification->user_id != Auth::user()->id){
                    $imageDataUri_join = 'data:' . $notification->profileImage_type  . ';base64,' . base64_encode( $notification->profileImage );
