...
class AdminController extends Controller
{
    ...
    public function user_auth_edit($userId,Request $request)
    {
        ...
        $user_p = UserProfile::find($userId) ? UserProfile::where('user_id', $userId)->first() : new UserProfile();

        $user_p_public = User_profile_public::find($userId) ? User_profile_public::where('user_id',$userId)->first() : new User_profile_public();
        ...
        $user_p->profile->social_links = $request->input('social_links');
        $user_p->profile->social_links2 = $request->input('social_links2');
        $user_p->profile->social_links3 = $request->input('social_links3');
        $user_p->profile->social_links4 = $request->input('social_links4');

        $user_p->favorite_articles_visibility = $request->input('favorite_private');


        if(UserProfile::find($userId)){
            $user_p->update();
        } else {
        ...
        if(User_profile_public::find($userId)){
            $user_p_public->update();
        } else {
            $user_p_public->user_id = $userId; 
            $user_p_public->save();
        }
        
        return redirect()->route('user_auth_data', ['userId' => $userId]);
    }
}