            ...
            return redirect('/login');
                ...
                $query4 = DB::table('great_arkwork')
                    ...
                    $query5 = DB::table('notify_join_mjoin')
                                ->select('notify_join_mjoin.*', 'shop_join.*', 'user_profile.profileImage', 'user_profile.profileImage_type', DB::raw('NULL as additional_field'), DB::raw('"notify_join_mjoin" as source'))
                                ->leftJoin('user as notify_user', 'notify_join_mjoin.user_id', '=', 'notify_user.id')
                                ->leftJoin('user as shop_user', 'shop_join.user_id', '=', 'shop_user.id')
                                ->leftJoin('user_profile', 'notify_join_mjoin.user_id', '=', 'user_profile.user_id')
                                ->where('notify_join_mjoin.user_id', '<>', Auth::user()->id)
                                ->where('shop_join.user_id', '<>', Auth::user()->id)
                                ->whereNotIn('notify_join_mjoin.user_id', [$currentUserId])
                                ->whereNotIn('shop_join.user_id', [$currentUserId])
                                ->whereNotIn('notify_join_mjoin.status', ['pass', 'reject'])
                                ->whereNotIn('shop_join.status', ['pass', 'reject'])
                                ->orderBy('updated_at', 'desc')
                                ->get();
