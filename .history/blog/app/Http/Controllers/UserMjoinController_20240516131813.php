...
class UserMjoinController extends Controller
{
    ...
    public function mjoin_post_form()
    {
        $check = UserPostMjoin::where('posted_by_u', Auth::user()->username)
                    ...
                    ->exists();
        $verity_phone = User::where('id',Auth::user()->id)->first();

        $check2 = User_join_mjoin::where('user_id', Auth::user()->id)
                    ->where('status', ['del', 'complete', 'end'])
                    ->first();
                    
        $shop_1 = UserPostMjoin::where('posted_by_u',Auth::user()->username)->get();
        //dd($check);
        if(!Auth::check()){
            return redirect('/login')->with('error', '請先登入');
        }
        if($verity_phone->verify == 'no'){
            $error_message = "請先完成驗證手機";
            return redirect()->route('verify_phone')->with('error', $error_message);
        }
        if($check){
            $error_message = "您已有一篇文章在招募了，請等待結束後才能再開啟揪團";
            return redirect()->route('front')->with('error', $error_message);
        }
        foreach($shop_1 as $shop_2){
            $shop_join = User_join_mjoin::where('article_id', $shop_2->id)->where('user_id', Auth::user()->id)->first();
            if($shop_2->status != 'pending' && empty($shop_join)){
        ...
        /*$mjoins = UserPostMjoin::orderBy('id', 'desc')
                           ...
                           foreach ($mjoins as $mjoin) {
                            ...
                            if(Auth::check()){
                                ...
                                ->leftJoin('mjoin', 'join_mjoin.article_id', '=','mjoin.id')
                                ->whereNotIn('join_mjoin.status', ['complete','pending','rejected'])
                            ...
                            $mjoin_all_member = DB::table('join_mjoin')
                                ...
                                $join_mjoin_you = UserPostMjoin::where('posted_by_u', Auth::user()->username)
                                ...
                                //dd($join_mjoin,$join_mjoin_you);
                                return view('auth.front', compact('mjoins','checks','mjoin_members','mjoin_all_member','join_mjoin','join_mjoin_you'));
                            }
    ...
    public function mjoin_solo($mjoinId)
    {
        $mjoin = DB::table('mjoin')
                               ...
                               ->first();
        if(!$mjoin){
            return redirect()->route('front')->with('error', '此揪團不存在或是遭到刪除');
        }
    ...
    public function mjoin_post_delete($mjoinId)
    {
        if(Auth::check()){
            $mjoindel = UserPostMjoin::where('id',$mjoinId)
                                       ->first();
            if(Auth::user()->username ===$mjoindel->posted_by_u){
                $status = 'del';
    ...
    public function mjoin_post_edit(Request $request,$mjoinId)
    {
        if(Auth::check()){
            ...
            if($current_time>$end_date){
                return redirect()->route('front')->with('error', '揪團已結束，無法編輯');
            }
    ...
    public function mjoin_reply($mjoinId)
    {
        ...
        $htmlContent = '<div class="SeeAllMessage">
                        <a href="/mjoin_solo/' . $mjoinId . '">查看全部留言</a>
                        </div>';
