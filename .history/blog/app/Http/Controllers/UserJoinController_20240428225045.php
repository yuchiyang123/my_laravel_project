...
$check = UserPostMjoin::where('posted_by_u', Auth::user()->username)
                      ->where('id', $mjoinId)
                      ->exists();
$check_repeat = UserJoinMjoin::where('user_id', Auth::user()->id)
                             ->where('article_id', $mjoinId)
                             ->exists();
$limit_join = UserJoinMjoin::where('user_id', Auth::user()->id)
                            ->whereNotIn('status', ['complete', 'exit'])
                            ->exists();

        dd($check_YoN,$check,$check_repeat,$limit_join);
        //登入判斷
        if(!Auth::check()){
