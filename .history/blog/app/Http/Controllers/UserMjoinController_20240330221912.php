...
class UserMjoinController extends Controller
{
    ...
    {
        $mjoin_replys = Mjoin_reply::where('reply_id', $mjoinId)->firse();

        //時間差運算
        $postTime = Carbon::parse($mjoin_replys->post_time);

        $currentTime = Carbon::now();

        $timeDifference = $currentTime->diffForHumans($postTime);

        $name = substr($mjoin_replys->user,0,1);
        ...
        <div>
            ...
            <div class="LeaveMessageall">
                <div class="LeaveMessageUsername">
                    <a href="#">'.$mjoin_replys->name.'</a>
                </div>
                <div class="LeaveMessageMain">
                    '.$mjoin_replys->main.'
                </div>
                <div class="LeaveMessageAction">
                    <a href="#">'.$timeDifference.'</a>&emsp;<a href="#">'.$mjoin_replys->good.'</a>&emsp;<a href="#">回復</a>
                </div>
            </div>
        </div>';
        return view('auth.front', compact('mjoin_replys'));
    }
