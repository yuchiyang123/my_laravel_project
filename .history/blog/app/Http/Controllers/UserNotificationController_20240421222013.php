    ...
    public function allnotify(){
        ...
        if(!Auth::check()){
            return redirect('/login');
                ...
                $query5 = DB::table('notify_join_mjoin')
                    ->select('notify_join_mjoin.*','mjoin.posted_by_u','user.profileImage', 'user.profileImage_type', DB::raw('NULL as additional_field'), DB::raw('"notify_join_mjoin" as source'))
        ...
        foreach ($notifications as $notification) {
            ...
            if ($notification->source === 'reply_artwork') {
                ...
                if($notification->status == 'pass' && $notification->posted_by_u == Auth::user()->username){
                    $imageDataUri_join = 'data:' . $notification->profileImage_type  . ';base64,' . base64_encode( $notification->profileImage );
                    $content_join = '您已被接受加入揪團';
                    $url_join = '';
                } elseif($notification->status =='reject' && $notification->posted_by_u == Auth::user()->username) {
                    $imageDataUri_join = 'data:' . $notification->profileImage_type  . ';base64,' . base64_encode( $notification->profileImage );
                    $content_join = '您已被拒絕加入揪團';
                    $url_join = '';
                } elseif ($notification->status =='pending' && $notification->posted_by_u == Auth::user()->username){
                    $imageDataUri_join = 'data:' . $notification->profileImage_type  . ';base64,' . base64_encode( $notification->profileImage );
