            ...
            return redirect('/login');
                ...
                $query4 = DB::table('great_arkwork')
                    ...
                    $query5 = DB::table('notify_join_mjoin')
                                ->select('notify_join_mjoin.*','mjoin.*', 'user.profileImage', 'user.profileImage_type', DB::raw('NULL as additional_field'), DB::raw('"notify_join_mjoin" as source'))
                                ->leftJoin('user', 'notify_join_mjoin.user_id', '=', 'user.id')
                                ->where('notify_join_mjoin.status', 'pending')
                ...
                if (Auth::user->username == $query5->posted_by_u ) {
                                    $query5 = $query5->where('notify_join_mjoin.status', '<>', 'pass')
