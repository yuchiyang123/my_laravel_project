...
class UserJoinController extends Controller
{
    ...
    public function join_shop_submit(Request $request,$shopId)
    {
        ...
        // 判斷是否為自己發表的揪團
        $check = UserPostWork::where('posted_by_u', Auth::user()->username)
                             ->where('id', $shopId)
                             ->first();

        // 判斷是否重複加入揪團
        $check_repeat = Shop_join::where('user_id', Auth::user()->id)
                                 ->where('article_id', $shopId)
                                 ->first();
        ...
        // 檢查必填字段是否有空值
        $requiredFields = ['name', 'email', 'contact_number', 'editor']; // 根據需求添加必填字段

        foreach ($requiredFields as $field) {
            if (!$request->filled($field)) {
                return back()->with('error', "{$field}不能為空");
            }
        }
