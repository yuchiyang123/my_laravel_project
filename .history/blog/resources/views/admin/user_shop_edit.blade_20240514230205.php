...
@endif
<form id="postForm" action="{{ route('user_cms_shop_edit_submit',['shopId' => $shopedits->id]) }}" method="POST">
    @csrf
    <div class="cut">
        <div class="grid-item">
            <div class="limt">
                ...
                <div class="mb-3 text-start">
                    ...
                    <select id="driver_license_requirements" name="driver_license_requirements" class="form-select form-select-sm" aria-label=".form-select-sm example">
                        <option value="不需要" @if($shopedits->driver_license_requirements == '不需要') selected @endif>不需要</option>
                        <option value="car" @if($shopedits->driver_license_requirements == 'car') selected @endif>開車</option>
                        <option value="motorcycle" @if($shopedits->driver_license_requirements == 'motorcycle') selected @endif>騎車</option>
                        <option value="both" @if($shopedits->driver_license_requirements == 'both') selected @endif>都需要</option>
                    </select>
                </div>
                <div class="mb-3 text-start ">
                    <label class="post-title-label fw-bolder">招募期間:</label>
                    <input type="text" id="timePicker" name="recruitment_period" class="form-control datepicker" placeholder="{{ $shopedits->recruitment_period }}" value="{{ $shopedits->recruitment_period }}">
                ...
                <div class="mb-3 text-start">
                    ...
                    <div class="form-check form-check-inline">
                        <input class="form-check-input" type="checkbox" value="中文" id="chineseCheckbox" name="language[]" {{ in_array('中文', $selectedLanguages) ? 'checked' : '' }}>
                        <label class="form-check-label" for="chineseCheckbox">中文</label>
                    </div>
                    <div class="form-check form-check-inline">
                        <input class="form-check-input" type="checkbox" value="日文" id="japaneseCheckbox" name="language[]" {{ in_array('日文', $selectedLanguages) ? 'checked' : '' }}>
                        <label class="form-check-label" for="japaneseCheckbox">日文</label>
                    </div>
                    <!-- 其他复选框省略 -->
                    <div class="form-check form-check-inline">
                        <input class="form-check-input" type="checkbox" value="韓文" id="koreanCheckbox" name="language[]" {{ in_array('韓文', $selectedLanguages) ? 'checked' : '' }}>
                        <label class="form-check-label" for="koreanCheckbox">韓文</label>
                    </div>
                    <div class="form-check form-check-inline">
                        <input class="form-check-input" type="checkbox" value="英文" id="englishCheckbox" name="language[]" {{ in_array('英文', $selectedLanguages) ? 'checked' : '' }}>
                        <label class="form-check-label" for="englishCheckbox">英文</label>
                    </div>
                    <div class="form-check form-check-inline">
                        <input class="form-check-input" type="checkbox" value="台語" id="taiwaneseCheckbox" name="language[]" {{ in_array('台語', $selectedLanguages) ? 'checked' : '' }}>
                ...
                <div class="LeaveMessageInput" style="color: black">
                    <textarea name="shop_information" id="editor" placeholder="">{!! $shopedits->shop_information !!}</textarea> <!-- CKEditor 编辑器 -->
                </div>
                <div class="mb-3 pt-3">
