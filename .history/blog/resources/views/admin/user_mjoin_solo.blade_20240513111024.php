                ...
                <div class="image-container">
                    <div class="d-block mb-3 text-start text-break">
                        ...
                        preg_match_all('/<img[^>]+src="([^">]+)"/', $mjoin->description, $matches); // 提取第一个图片
                            ...
                            @endphp
                            @if(strlen($text) > 150)
                            <p>{!! $mjoin->description !!}</p>
                            
                            @else
                            <p>{!! $mjoin->description !!}</p>
                            @endif
                            @if ($images)
                            <div id="carouselExampleControlsNoTouching{{ $mjoin->id }}" class="carousel carousel-dark slide" data-ride="carousel" data-interval="false">
                                ...
                                @if($imgcount > 2)
                                <a class="carousel-control-prev" href="#carouselExampleControlsNoTouching{{ $mjoin->id }}" role="button" data-slide="prev">
                                    <span class="carousel-control-prev-icon" aria-hidden="true"></span>
                                    <span class="sr-only">Previous</span>
                                </a>
                                <a class="carousel-control-next" href="#carouselExampleControlsNoTouching{{ $mjoin->id }}" role="button" data-slide="next">
