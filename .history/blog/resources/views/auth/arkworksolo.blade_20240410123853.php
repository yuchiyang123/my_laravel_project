            ...
            <div class="line">
                <div class="inner-grid">
                    <form id="like-form" action="{{ route('posts.like', ['reply_id' => $userarkworksolo->id, 'great_code' => 'artwork']) }}" method="POST">
                        @csrf
                        @if(!$usersologood)
                            <a type="submit" class="btn btn-primary like-button" data-reply-id="{{ $userarkworksolo->id }}">讚啦</a>
                        @elseif($usersologood->many=="1")
                            <button type="submit" class="btn btn-primary like-button" data-reply-id="{{ $userarkworksolo->id }}">取消</button>
                        @else
                            <button type="submit" class="btn btn-primary like-button" data-reply-id="{{ $userarkworksolo->id }}">讚啦</button>
                        @endif
                    </form>
                </div>
                <div class="inner-grid">
