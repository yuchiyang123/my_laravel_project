            ...
            <div class="limt">
                ...
                <div class="line">
                    ...
                    @if(\App\Models\MjoinScore::where('rater_id', Auth::user()->id)->first() == null)
                        @if(\App\Models\UserPostMjoin::select('status')->where('posted_by_u', Auth::user()->username)->first()->status == 'complete')
                        ...
                        @elseif(\App\Models\UserPostMjoin::select('status')->where('posted_by_u', Auth::user()->username)->first()->status == 'pending')
                            <a href="/score_complete/{{ $mjoin->id }}"><button type="button" class="btn btn-primary" data-bs-toggle="tooltip"
                                 data-bs-placement="top" title="要按完成才能評分喔">完成</button></a>
                        @endif
                    @endif
                    ...
                    <div class="inner-grid">
                        <a href="#" id="endexampleModal" data-bs-target="#endexampleModal" data-mjoin-id="{{ $mjoin->id }}">結束揪團</a>
                    </div>
                    <div class="modal fade" id="endexampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true" style="z-index: 99999;">
                        <div class="modal-dialog mx-auto">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="exampleModalLabel">結束揪團</h5>
                                ...
                                <div class="modal-footer">
                                    ...
                                    <form action="" method="POST">
                                        @csrf
                                        <button type="submit" class="btn btn-primary">確定結束</button>
                                    </form>
                                </div>
                            </div>
                        </div>
