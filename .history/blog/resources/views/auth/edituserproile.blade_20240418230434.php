...
$('#upload-input').change(function(e) {
    if (this.files && this.files[0]) {
        ...
        reader.onload = function(event) {
            ...
            image.onload = function() {
                ...
                $('#crop-button').show();
                console.log('图片对象加载完成');
                // 初始化 Cropper
                var cropper = new Cropper(image, {
    aspectRatio: 1, // 设置裁剪框宽高比为1:1，确保裁剪区域为正圆形
    viewMode: 1, // 设置裁剪框视图模式
    autoCropArea: 1, // 默认裁剪框自动选择整个图片
    ...
    crop: function(event) {
        ...
        var scaleX = cropBoxData.width / canvasData.width;
        var scaleY = cropBoxData.height / canvasData.height;

        var x = cropBoxData.left - canvasData.left;
        var y = cropBoxData.top - canvasData.top;
        var width = cropBoxData.width;
        var height = cropBoxData.height;

        if (width < height) {
            height = width;
            y += (cropBoxData.height - height) / 2;
        } else {
            width = height;
            x += (cropBoxData.width - width) / 2;
        }
