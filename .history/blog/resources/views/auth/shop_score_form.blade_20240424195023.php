...
@section('content')
<form method="GET" action="/shop_score_form/submit/{{ $shop_author_1->id }}">
    @csrf
...
<div style="text-align: center; ">
    <label for="title" class="FW-bold mb-2" style="font-size: 25PX">打工店家評價</label>
    <div><label for="title" class="d-flex justify-content-start FW-bold" style="font-size: 20px">評價打工店家-{{ $shop_author_1->shop_name }}</label></div>
    @if(Auth::user()->username != $shop_author_1->posted_by_u)
        <div class="score-container d-flex justify-content-start pt-2  border-top border-2"><h3 class="fw-bolder">主揪人:{{ $shop_author_1->posted_by_u }}</h3></div>
        <div class="feedback d-flex justify-content-center m-1 " >
            <label class="angry">
                @php
                $user_id = DB::table('user')->where('username', $shop_author_1->posted_by_u)->value('id');
            ...
            <label class="ok">
                <input type="radio" value="3" name="feedback[{{ $user_id }}]" />
                <div></div>
            </label>
            <label class="good">
                <input type="radio" value="4" name="feedback[{{ $user_id }}]" checked />
                <div>
                    <svg class="eye left">
                        <use xlink:href="#eye">
                    </svg>
            ...
            <label class="happy">
                <input type="radio" value="5" name="feedback[{{ $user_id }}]" />
                <div>
                    <svg class="eye left">
                        <use xlink:href="#eye">
                    </svg>
        ...
        <div class="LeaveMessageInput flex-row border-bottom border-2">
            <div class="p-2">
                <textarea class="form-control" placeholder="詳細說明這旅程與他想處的經過" id="comments_{{ $shop_author_1->posted_by_u }}" 
                    name="comments_{{ $shop_author_1->posted_by_u }}" rows="3
                "></textarea>
            </div>
        </div>
        @endif
    ...
    @php
        $join_member_name = DB::table('user')->where('id', $join_member->user_id)->value('username');
    @endphp
