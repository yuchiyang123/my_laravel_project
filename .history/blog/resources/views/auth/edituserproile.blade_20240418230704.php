...
$('#upload-input').change(function(e) {
    ...
    guides: false, // 不显示裁剪框的辅助线
    background: false, // 不在裁剪框背后显示背景
    modal: true, // 显示裁剪框的模态对话框
    scalable: true, // 图片是否可以缩放
});

// 当 Cropper 创建完成后，手动调整裁剪框为圆形
cropper.setCropBoxData({
    left: 100, // 指定裁剪框左上角的 x 坐标
    top: 100, // 指定裁剪框左上角的 y 坐标
    width: 300, // 指定裁剪框的宽度
    height: 300, // 指定裁剪框的高度
});
...
cropper.setCropBoxData({
    width: 300, // 设置裁剪框的宽度
    height: 300, // 设置裁剪框的高度
    left: (cropper.getContainerData().width - 300) / 2, // 计算裁剪框的左上角 x 坐标
    top: (cropper.getContainerData().height - 300) / 2, // 计算裁剪框的左上角 y 坐标
    // 将裁剪框修改为圆形
    borderRadius: '50%',
});
