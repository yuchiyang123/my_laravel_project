            ...
            <div class="line">
                <div class="inner-grid">
                    <!--form id="like-form" action="{{ route('posts.like', ['reply_id' => $userarkworksolo->id, 'great_code' => 'artwork']) }}" method="POST"-->
                        @csrf
                        @if(!$usersologood)
                            <button type="submit" class="ui icon button like-button" data-reply-id="{{ $userarkworksolo->id }}" data-reply-greatcode="artwork">
                                <ion-icon name="thumbs-up-outline">讚</ion-icon>
                            </button>
                        @elseif($usersologood->many=="1")
                            <button type="submit" style="font-size: 20px;" class="ui icon like-button" data-reply-id="{{ $userarkworksolo->id }}" data-reply-greatcode="artwork">
                                <ion-icon name="thumbs-up"></ion-icon>讚
                            </button>
                        @else
                            <button type="submit" style="font-size: 20px;" class="ui icon button like-button" data-reply-id="{{ $userarkworksolo->id }}" data-reply-greatcode="artwork">
        ...
        $('.like-button').click(function() {
            ...
            $.ajax({
                ...
                success: function(data) {
                    if (data.liked) {
                        $('.like-button[data-reply-id="' + replyId + '" , reply-greatcode="' + greatcode + '"] ion-icon').attr('name', 'thumbs-up');
                    } else {
                        $('.like-button[data-reply-id="' + replyId + '" , reply-greatcode="' + greatcode + '"] ion-icon').attr('name', 'thumbs-up-outline');
                    }
                },
