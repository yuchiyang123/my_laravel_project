...
<div class="cut">
    <div class="grid-item">
        <div class="limt">
            ...
            <div class="post_condition fw-bolder border-bottom border-top pt-2 pb-2 mt-1 mb-1 flex-row bd-highlight" style="font-size: 14px;">
                <label class="bd-highlight">日期:{{ $mjoin->time }}</label><label class="bd-highlight">人數:{{ $mjoin->people }}</label>
                <label>預算:{{ $mjoin->money }}</label><label>性別:{{ $mjoin->sex }}</label>
                <label>需要技能:{{ $mjoin->skill }}</label><label>年齡:{{ $mjoin->age }}</label>
            </div>
