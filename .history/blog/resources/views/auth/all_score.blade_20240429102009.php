                                                ...
                                                <li class="list-group-item d-flex justify-content-between align-items-center">
                                                    <div class="flex-grow-1">
                                                        @if($score == $usershopscore)
                                                         @if(!empty($item->comments)){{ $item->comments }}@else 無評論@endif<span class="badge bg-danger"> {{ $item->score }}</span>
                                                        @elseif($score == $usermjoinscore)
                                                         @if(!empty($item->comments)){{ $item->comments }}@else 無評論@endif <span class="badge bg-danger"><span>分數:</span>{{ $item->score }}</span>
                                                        @endif
                                                    </div>
                                                    <div class="text-end">
