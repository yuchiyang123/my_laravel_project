...
<div class="cut">
    <div class="grid-item">
        <div class="limt">
            ...
            <div class="line">
                <a href="#" onclick="good({{ $mjoin->id }})" data-mjoin-id="{{ $mjoin->id }}" class="like-button">
                ...
                <script>
                    $(document).ready(function() {
                        ...
                        likeButtons.each(function() {
                            var button = $(this);
                            var mjoinId = button.data('mjoin-id');
                            // 发送 AJAX 请求来检查用户的点赞状态
                            $.ajax({
                                type: 'GET',
                                url: '/mjoin_post_posts/checkgood/' + {{ $mjoin->id }},
                                
                                success: function(response) {
                                    if (response.status === 'true') {
                                        // 如果用户已经点赞，添加 liked 类来改变按钮的样式
                                        button.addClass('liked');
                                        button.find('.material-icons').text('thumb_up');
                                        button.find('.fs-6').text('讚');
                                    } else {
                                        button.removeClass('liked');
                                        button.find('.material-icons').text('thumb_up_off_alt');
                                        button.find('.fs-6').text('讚');
                                    }
                                },
