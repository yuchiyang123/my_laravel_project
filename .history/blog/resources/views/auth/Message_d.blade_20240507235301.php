     ...
     Echo.channel('chat{{ Auth::user()->id }}{{ $receiver_id }}' || 'chat{{ $receiver_id }}{{ Auth::user()->id }}')
         .listen('msg', (e) => {
             ...
             if(e.fromUserId == '{{ Auth::user()->id }}')
                $('.position-relative').append('<div style="display: flex; justify-content: flex-end;" id="message_show_d{{ $message->id }}">'+'<div class="chat-messages p-4">' + e.message + '</div>'+'</div>')
                //$('#message_show_d{{ $message->id }}').css({"display": "flex", "justify-content": "flex-end"}).append('<div class="chat-messages p-4">' + e.message + '</div>');
             else
                $('#message_show_d{{ $message->id }}').css({"display": "flex", "justify-content": "flex-start"}).append('<div class="chat-messages p-4">' + e.message + '</div>');
