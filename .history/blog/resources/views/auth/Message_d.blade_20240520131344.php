...
<main class="content">
        ...
        <div class="card vh-100 d-flex">
            <div class="row g-0">
                <div class="">
                    ...
                    <div class="position-relative overflow-auto" id="position-relative_msg" style="max-height: 750px; overflow-y: auto; height: calc(100% - 50px);">
                        @foreach($messagese as $message)
                            @if($message->user_id == Auth::user()->id)
                                <div class="d-flex justify-content-end me-2" id="message_show_db{{ $message->id }}">
                                    <div class="chat-message bg-primary text-white p-3 rounded-pill mb-2">{{ $message->message }}</div>
                                </div>
                            @else
                                <div class="d-flex justify-content-start ms-2" id="message_show_db{{ $message->id }}">
                                    <div class="chat-message bg-light text-dark p-3 rounded-pill mb-2">{{ $message->message }}</div>
                                </div>
                            @endif
                        @endforeach
                    </div>
                    <div class="position-fixed bottom-0 bg-white p-3" style="width: 100%">
    ...
    .position-relative {
        height: calc(100% - 50px); /* 确保消息列表填充剩余空间 */
    }
    </style>
        ...
        Echo.channel('chata{{ $receiver_id }}{{ Auth::user()->id }}')
            .listen('msg', (e) => {
                ...
                if (e.fromUserId == '{{ Auth::user()->id }}') {
                    $('#position-relative_msg').append('<div class="d-flex justify-content-end me-2" id="message_show_db{{ $receiver_id }}">' + '<div class="chat-message bg-primary text-white p-3 rounded-pill mb-2">' + e.message + '</div>' + '</div>');
                } else if (e.fromUserId != '{{ Auth::user()->id }}') {
                    $('#position-relative_msg').append('<div class="d-flex justify-content-start ms-2" id="message_show_db{{ $receiver_id }}">' + '<div class="chat-message bg-light text-dark p-3 rounded-pill mb-2">' + e.message + '</div>' + '</div>');
                }
            });
