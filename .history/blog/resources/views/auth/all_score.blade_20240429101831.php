                                                ...
                                                <li class="list-group-item d-flex justify-content-between align-items-center">
                                                    <div class="flex-grow-1">
                                                        @if($score == $usershopscore)
                                                         @if(!empty($item->comments))<span class="badge bg-danger">{{ $item->comments }}</span>@endif {{ $item->score }}
                                                        @elseif($score == $usermjoinscore)
                                                         @if(!empty($item->comments))<span class="badge bg-danger">{{ $item->comments }}</span>@endif {{ $item->score }}</a>
                                                        @endif
                                                    </div>
                                                    <div class="text-end">
