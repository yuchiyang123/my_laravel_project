        ...
        <div class="grid-item">
            <div class="limt">
                ...
                <div class="line">
                    ...
                    <!-- 显示评分按钮 -->
                    @php
                        $userMjoinScore = \App\Models\MjoinScore::where('rater_id', Auth::user()->id)->exists();
                        $userJoinMjoinStatus = \App\Models\UserJoinMjoin::where('user_id', Auth::user()->id)
                                                                        ->where('article_id', $mjoin->id)
                                                                        ->value('status');
                        $userPostMjoinStatus = \App\Models\UserPostMjoin::where('posted_by_u', Auth::user()->username)
                                                                        ->value('status');
                    @endphp

                    @if(!$userMjoinScore && $userJoinMjoinStatus == 'pass' || $userJoinMjoinStatus == 'complete')
                        @if($userPostMjoinStatus == 'complete' || $userPostMjoinStatus == 'end')
