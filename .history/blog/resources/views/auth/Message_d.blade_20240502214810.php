			...
			<div class="row g-0">
				<div class="">
					...
					<div class="position-relative" >
						...
						<div class="chat-messages p-4 " id="message_show_d{{ $message->id }}">{{ $message->message }}</div>
                        @endforeach
                    </div>
                    <div>
				</div>
   ...
   $(document).ready(function() {
    ...
    Echo.channel('chat{{ Auth::user()->id }}{{ $receiver_id }}')
        ...
        .listen('msg', (e) => {
            console.log(e);
            // 更新界面上對應帖子的點讚數顯示
            $('#message_show_d').html('<div class="chat-messages p-4" id="message_show_d{{ $message->id }}">' + e.message + '</div>');
        });
});
</script>
