...
@section('content')
        ...
        <form>
            ...
            <button type="button" class="btn btn-primary mt-3" onclick="user_phone();">發送簡訊</button>
            ...
            <form>
                ...
                <button type="button" class="btn btn-danger mt-3" onclick="user_phone_verify()">驗證</button>
            </form>
            </div>
            ...
            $.ajax({
                ...
                success: function(response) {
                    if(response.status == 'OK'){
                        ...
                        console.log(response.status);
                        $("#error").text('這支電話已經驗證過了，請換另一個電話驗證。');
                        $("#error").show();
                    }
                }
            })
        }
        function user_phone_verify(){
            var number = $("#number").val();
            $.ajax({
                url:"/user_phone_verify/"+ number ,
                type: "GET",
                success: function(response) {
                    if(response.status == 'OK'){
                        verify();
