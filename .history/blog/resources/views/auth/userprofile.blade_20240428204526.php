                    ...
                    <div class="row" id="all-projects">
                        @if(!empty($userCollections) || !empty($userCollections_mjoin|| !empty($userCollections_shop)))
                            @foreach([$userCollections, $userCollections_mjoin ,$userCollections_shop] as $collection)
                                @php
                                    // 将所有集合中的项目合并为一个数组并按照创建时间逆序排序
                                    $mergedCollections = collect([$userCollections, $userCollections_mjoin, $userCollections_shop])->flatten()->sortByDesc('created_at')->take(2);
                                @endphp
                                @foreach($mergedCollections as $item)
