        ...
        <div class="grid-item">
            <div class="limt">
                ...
                <div class="post_condition fw-bolder border-bottom border-top pt-2 pb-2 mt-1 mb-1 d-flex flex-row bd-highlight text-break" style="font-size: 14px;top:-30px;position:relative;">
                    <div><label class="flex-fill ps-3">店名:{{ $shops->shop_name }}</label>
                    <label class="flex-fill ps-3">招募期間:{{ $shops->recruitment_period }}</label>
                    <label class="flex-fill">縣市:{{ $shops->selectwhere }}</label></div>
                    <div><label class="flex-fill">地址:{{ $shops->location }}</label></div>
                    <div><label class="flex-fill">性別:{{ $shops->sex }}</label>
                    <label class="flex-fill">工作經驗:{{ $shops->conditions_exp }}</label>
                    <label class="flex-fill">每日工時:{{ $shops->work_hours }}</label></div>
