...
<div style="text-align: center; ">
    ...
    @if(Auth::user()->username != $mjoin_author_1->posted_by_u)
        ...
        <div class="feedback d-flex justify-content-center m-1 " >
            <label class="angry">
                @php
                $user_id = DB::table('user')->where('username', $mjoin_author_1->posted_by_u)->value('id');
        ...
        @php 
        $user_id = DB::table('user')->select('id')->where('username', $mjoin_author_1->posted_by_u)->value('id');
        @endphp
        @if(session('user_name')!=null)                    
        @csrf
        <div class="LeaveMessageInput flex-row border-bottom border-2">
            <div class="p-2">
                <textarea class="form-control" placeholder="詳細說明這旅程與他想處的經過" id="comments[{{ $mjoin_author_1->posted_by_u }}]" 
                    name="comments[{{ $user_id }}]" rows="3
                "></textarea>
            </div>
        </div>
        @endif
...
<div class="LeaveMessageInput flex-row border-bottom border-2">
    <div class="p-2">
        <textarea class="form-control" placeholder="詳細說明這旅程與他想處的經過" id="comments[{{ $join_member->user_id }}]" name="comments[{{ $join_member->user_id }}]" rows="3
        "></textarea>
    </div>
</div>
@endif
