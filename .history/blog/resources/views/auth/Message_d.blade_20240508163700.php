        ...
        Echo.channel('chata{{ $receiver_id }}{{ Auth::user()->id }}')
            .listen('msg', (e) => {
                ...
                if(e.fromUserId == '{{ Auth::user()->id }}'){
                    $('#position-relative_msg').append('<div style="display: flex; justify-content: flex-end;" id="message_show_d{{ $receiver_id }}">'+'<div class="chat-messages p-4">' + e.message + '</div>'+'</div>')
                }
                    
                    
                else if(e.fromUserId != '{{ Auth::user()->id }}'){
                    $('#position-relative_msg').append('<div style="display: flex; justify-content: flex-start;" id="message_show_d{{ $receiver_id }}">'+'<div class="chat-messages p-4">' + e.message + '</div>'+'</div>')
                }
