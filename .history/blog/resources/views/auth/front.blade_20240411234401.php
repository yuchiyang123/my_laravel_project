            ...
            <div class="limt">
                ...
                <div class="container">
                    <div class="respond">
                        ...
                        <a href="#">👍🏽</a>
                        <div>
                            <a href="#" data-mjoin-count="{{ $mjoin->id }}" class="goodcount_{{ $mjoin->id }}"><span class="goodcount_{{ $mjoin->id }}"></span></a>
                        </div>
                    </div>
                ...
                <div class="ShowAllMessage">
                    <script>
                        ...
                        function updateGoodCount(mjoinId) {
                            mjoin_post_countgood(mjoinId); // 调用获取点赞数量的函数

                            // 设置每秒钟执行一次获取点赞数量的函数
                            setInterval(function() {
                                mjoin_post_countgood(mjoinId);
                            }, 500); // 1000 毫秒等于 1 秒
                        }
                        var link = $(this);
                        var mjoinId = link.data('mjoin-count');
                        // 调用函数来初始化和启动点赞数量的更新
                        updateGoodCount(mjoinId);
                        //取消
                    </script>
                </div>
            ...
            links.each(function() {
                ...
                link.click(function(event) {
                    ...
                    if (link.hasClass('liked')) {
                        ungood(mjoinId, link); // 调用取消喜欢的函数
                    } else {
                        good(mjoinId, link); // 调用喜欢的函数
                    }
                });
            });
        });
