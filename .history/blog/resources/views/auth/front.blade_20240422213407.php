                ...
                <div class="line">
                    ...
                    @if(\Carbon\Carbon::now()->gt($end_date) && session('user_name')!=null && 
                        ...
                        @if(\App\Models\MjoinScore::where('rater_id', Auth::user()->id)->first() == null){
                            @if(\App\Models\UserPostMjoin::select('status')->where('posted_by_u', Auth::user()->username)->first() == 'complete'){
                                <a href="/score_form/{{ $mjoin->id }}"><button type="button" class="btn btn-primary">評分</button></a>
                            }@elseif(\App\Models\UserPostMjoin::select('status')->where('posted_by_u',Auth::user()->username)->first() == 'pending'){
