            ...
            <div class="line">
                <div class="inner-grid">
                    <!--form id="like-form" action="{{ route('posts.like', ['reply_id' => $userarkworksolo->id, 'great_code' => 'artwork']) }}" method="POST"-->
                        @csrf
                        @if(!$usersologood)
                            <button type="button" class="ui icon button like-button" data-reply-id="{{ $userarkworksolo->id }}" data-reply-greatcode="artwork">
                                <ion-icon name="thumbs-up-outline">讚</ion-icon>
                            </button>
                        @elseif($usersologood->many=="1")
                            <button type="button" style="font-size: 20px;" class="ui icon like-button" data-reply-id="{{ $userarkworksolo->id }}" data-reply-greatcode="artwork">
                                <ion-icon name="thumbs-up"></ion-icon>讚
                            </button>
                        @else
                            <button type="button" style="font-size: 20px;" class="ui icon button like-button" data-reply-id="{{ $userarkworksolo->id }}" data-reply-greatcode="artwork">
                                <ion-icon name="thumbs-up-outline"></ion-icon>讚
                            </button>
                        @endif
                    <!--/form-->
                </div>
...
<script>
     $(document).ready(function() {
        $('.like-button').click(function() {
            ...
            var greatcode = $(this).data('reply-greatcode');
            var formData = {
                'reply_id': replyId,
                'greatcode': greatcode,
                '_token': '{{ csrf_token() }}'
            ...
            $.ajax({
                type: 'POST',
                url: '/posts/{{ $userarkworksolo->id }}/artwork/like',
                data: formData,
                dataType: 'json',
                success: function(data) {
                    if (data.liked) {
                        $('.like-button[data-reply-id="' + replyId + '"][data-reply-greatcode="' + greatcode + '"] ion-icon').attr('name', 'thumbs-up');
                    } else {
                        $('.like-button[data-reply-id="' + replyId + '"][data-reply-greatcode="' + greatcode + '"] ion-icon').attr('name', 'thumbs-up-outline');
                    }
                },
