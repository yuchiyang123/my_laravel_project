...
$(document).ready(function() {
    ...
    if(message !== 'null') {
        ...
        .listen('msg_outside', (e) => {
            console.log(e);
            // Construct new message HTML element
            var newMessage = `
            ...
            // Check if a message with the same receiver_id already exists on the page
            var existingMessage = $('#message_show_d' + e.toUserId);
            console.log(existingMessage);
            if(existingMessage.length > 0) {
                // If it exists, update the message content
                existingMessage.html(newMessage);
            } else {
                // Otherwise, prepend the new message to the top of the container
