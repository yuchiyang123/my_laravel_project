...
<div class="outside">
    ...
    <script>
        ...
        function submitReply(mjoinId) {
                        ...
                        function showReply_all(mjoinId) {
                            $.ajax({
                                url: "/front-reply/all/" + mjoinId,
                                type: "GET",
                                success: function(response) {
                                    // 在彈出的視窗中顯示留言
                                    $('#showAllMessage1_' + mjoinId).html(response.htmlContent1);
                                    $('#showAllMessage1_' + mjoinId).show();
                                },
                                error: function(xhr, status, error) {
                                    console.error(xhr.responseText);
                                }
        ...
        function showPopup(mjoinId) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $('.popup-content').html(response.htmlContent1);
                    $('.popup').show();
                    $('#showAllMessage1_' + mjoinId).html(showReply_all(mjoinId));
                },
                error: function(xhr, status, error) {
                    console.error(xhr.responseText);
                }
