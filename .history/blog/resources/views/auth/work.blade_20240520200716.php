        ...
        <div class="grid-item">
            <div class="limt">
                ...
                <div class="line">
                    @if(Auth::check())
                    @php
                        $ex = App\Models\Shop_join::where('article_id', $shop->id)->whereIn('status',['pass','end'])->where('user_id', Auth::user()->id)->first();
                        //dd($ex);
                        $ex1 = App\Models\WorkScore::where('rater_id', Auth::user()->id)->where('article_id', $shop->id)->first();
                    @endphp
                    @endif
                    @if(Auth::check())
                    @if($ex && !$ex1)
                    ...
                    <!-- 显示评分按钮 -->
                        @if(\App\Models\UserPostWork::where('id',$shop->id)->whereIn('status',['end','pass','complete'])->first())
                    ...
                    @endphp
                    @endif
                    @if(session('user_name')!=null && session('user_name')!=$shop->posted_by_u && !$exists &&!\Carbon\Carbon::now()->gt($end_date) && !$exists1 && !$exists2)
