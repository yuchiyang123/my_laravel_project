        ...
        <div class="grid-item">
            <div class="limt">
                ...
                <div class="text-container">
                    ...
                    <div class="date">
                        <!--貼文日期-->
                        <div><a href="/mjoin_solo/{{ $mjoin->id }}">{{ $mjoin->date }}</a></div>
                    </div>
                    <div class="d-flex justify-content-end">
                        <img style="width: 120px; margin-top: -80px;" src="{{ asset('image/停止招募.png') }}" alt="">
                    </div>
                </div>
                <div class="post_condition fw-bolder border-bottom border-top pt-2 pb-2 mt-1 mb-1 d-flex flex-row bd-highlight text-break" style="font-size: 14px;top:-50px;position:relative;">
                ...
                <div class="line">
                    ...
                    @php
                        if(Auth::check()){
                            ...
                            $user_post_mjoin = DB::table('mjoin')->where('id',$mjoin->id)->where('posted_by_u',Auth::user()->username)->where('status','<>','complete')->exists();

                            $user_post_mjoin_join = DB::table('join_mjoin')->where('article_id',$mjoin->id)->exists();
                        }
                    @endphp
                    @if(Auth::check())
                    @if($user_post_mjoin && $user_post_mjoin_join)
                    ...
                    @if( Auth::check()&&(!$userMjoinScore && $userJoinMjoinStatus == 'pass' ))
                        @if($userPostMjoinStatus == 'end')
                            <a href="/score_form/{{ $mjoin->id }}">
                                <button type="button" class="btn btn-primary" data-bs-toggle="tooltip"
                                        data-bs-placement="top" title="要完成評分喔">評分</button>
                            </a>
                        @else
                            <span>活動未結束還不能評分喔</span>
                        @endif
                    @endif
                    ...
                    @php
                        if(Auth::check()){
                            $user_join = DB::table('join_mjoin')->where('article_id',$mjoin->id)->where('user_id',Auth::user()->id)->exists();
                        }
                    @endphp
                    ...
                    @if(session('user_name')!=null)
                        @if(session('user_name')!=$mjoin->posted_by_u) 
                            @if(!\Carbon\Carbon::now()->gt($end_date))
                                @if(!$user_join)
                                    <div class="inner-grid">
                ...
                <div class="ShowAllMessage">
                    <script>
                        ...
                        /*function showReply(mjoinId){
                            // 監聽消息
                            socket.on('frontreply2'+ mjoinId, (htmlContent) => {
                                console.log('收到消息:', htmlContent);
                                $('#showAllMessage_' + mjoinId).html(htmlContent);
                                $('#showAllMessage_' + mjoinId).show();
                                // 在這裡處理收到的消息，例如更新 UI
                            });
                        }*/
    ...
    @if(Auth::check())
            ...
            <div class="list-group">
                ...
                @foreach ($join_mjoin as $mjoin)
                    @if (!$titleDisplayed)
                        <a href="/mjoin_solo/{{ $mjoin->mjoin_id }}" class="list-group-item list-group-item-action">{{ $mjoin->title }}</a>
                        @php $titleDisplayed = true; @endphp
                    @endif
                    @php
                        $user_name = DB::table('user')->where('id', $mjoin->user_id)->value('username');
                    @endphp
                    <a href="/user-profile/index/d/{{ $user_name }}" class="list-group-item list-group-item-action">{{ $user_name }}</a>
                @endforeach
            </div>
            <br>
        
            <div class="list-group" id='join_view' style="width: 15%ㄍ">
                <label for="joinmjoin_usermember">你參加的團隊</label>   
                @foreach ($mjoin_members as $mjoin)
                    @php $titleDisplayed = false; @endphp
                    <a href="/mjoin_solo/{{ $mjoin->id }}" class="list-group-item list-group-item-action">{{ $mjoin->title }}</a>
                        @php $titleDisplayed = true; @endphp
                    <a href="/user-profile/index/d/{{ $mjoin->posted_by_u }}" class="list-group-item list-group-item-action">{{ $mjoin->posted_by_u }}</a>
                @endforeach
