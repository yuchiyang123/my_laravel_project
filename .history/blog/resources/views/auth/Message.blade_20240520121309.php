...
<div class="">
    ...
    @foreach ($messages as $message)
        @php
            ...
            if (in_array($message->user_id, $processed_receiver_ids) || in_array($message->receiver_id, $processed_receiver_ids) || in_array($message->receiver_id, $processed_user_ids) || in_array($message->user_id, $processed_user_ids)){
                continue;
            }
            ...
            $username = App\Models\User::where('id', $userid)->first();
        ...
        <a href="/message-view-show/{{ $userid }}" >
            <div id= "message_container">
                <div class="d-flex flex-row align-items-center m-3" id="message_show_d{{ $userid }}">
                    @if($user_img->profileImage_type != null)
                        <img src="{{ $imageDataUri }}" style="width: 50px;" class="user_img_message rounded-circle" id="l_{{ $userid }}">
                    @else
                        <img src="https://github.com/mdo.png" style="width: 50px;" class="user_img_message rounded-circle" id="l_{{ $userid }}">
                    @endif
                    <div class="">
                        <div class="d-flex align-items-center fw-bold UserMassageBoxName">
                            <span class="ps-2">{{ $username->username }}</span>
                        </div>
                        <div class="d-flex align-items-center UserMassageBoxMessage">
                            <span class="ps-2">{{ $message->message }} </span>
...
$(document).ready(function() {
        ...
        .listen('msg_outside', (e) => {
            //$id = ${e.toUserId};
            //console.log(e);
            //$username = \App\modules\User::where('id',)->first()
            // 构造新消息的 HTML 元素
            var newMessage = `
                ...
                <div class="d-flex flex-row align-items-center" id="message_show_d${e.toUserId}">
                        ...
                        <img src="https://github.com/mdo.png" style="width: 50px;" class="user_img_message rounded-circle" id="l_${e.toUserId}">
                    
                    <div class="">
                        <div class="d-flex align-items-center fw-bold UserMassageBoxName ms-2">
                            <span>${e.user_name}</span>
                        </div>
                        <div class="d-flex align-items-center UserMassageBoxMessage ms-2">
                            <span>${e.message}</span>
            ...
            if(existingMessage.length > 0) {
                ...
                existingMessage1.html(newMessage);
            }
            else {
                // 否则将新消息添加到容器的顶部
