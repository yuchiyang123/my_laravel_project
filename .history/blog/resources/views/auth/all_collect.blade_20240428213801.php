                                        ...
                                        <ul class="list-group" style="width: 750px">
                                            <li class="list-group-item d-flex justify-content-between align-items-center">
                                                <div>
                                                    ...
                                                    <span class="badge bg-primary rounded-pill ms-1">創作</span>
                                                    @elseif($collection == $userCollections_mjoin)
                                                    <a href="{{ route('mjoin_solo', ['mjoinId' => $item->id]) }}">{{ $item->title }}</a>
                                                    <span class="badge bg-primary rounded-pill ms-1">揪團</span>
                                                    @else
                                                    <a href="{{ route('shop_solo', ['shopId' => $item->id]) }}">{{ $item->shop_name }}</a>
                                                    <div class="text-end">{{ $item->created_at }}</div>
                                                    <span class="badge bg-primary rounded-pill ms-1">打工</span>
                                                    @endif
                                                </div>
