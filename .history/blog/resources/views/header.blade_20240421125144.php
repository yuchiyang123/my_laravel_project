...
/* CSS */
.notification-button {
    background-color: #f2f2f2; /* 这里是通知按鈕的颜色 */
    border-radius: 50%; /* 使按钮呈现圆形 */
}

.notification-panel {
    display: none; /* 默认情况下隐藏通知面板 */
    position: absolute;
    top: 60px; /* 位置根据你的需要进行调整 */
    right: 20px; /* 位置根据你的需要进行调整 */
    width: 250px; /* 通知面板的宽度 */
    background-color: #ffffff; /* 通知面板的颜色 */
...
<header class="header">
    <div >
        <div class="headerdiv">
            ...
            <div ></div>
            <div >
                <div class="notification-button" style="width: 45px; height: 45px;"><span class="material-icons">
                    notifications
                    </span></div>
                <!-- 通知面板 -->
                <div class="notification-panel" id="notificationPanel">
                    <div class="normal-notifications" id="normalNotifications">
                        <!-- 普通的用戶通知 -->
                    </div>
                    <div class="team-notifications" id="teamNotifications">
                        <!-- 申請加入團隊的通知 -->
                    </div>
                </div>
            </div>
...
<script>
    ...
    // 然后，你可以使用 JavaScript 进行动态地向普通用户通知和团队通知面板中添加通知。
    // 这里我们使用 JavaScript 向普通用户通知面板中添加一个示例通知。
    var normalNotifications = document.getElementById('normalNotifications');
    var notification1 = document.createElement('div');
    notification1.textContent = '有人按贊了你的貼文';
    normalNotifications.appendChild(notification1);

    // 同样的方法可以应用到团队通知面板。
    // 更多的 JavaScript 代码可以根据你的需求进行添加。
