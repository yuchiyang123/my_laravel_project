...
class UserMessageController extends Controller
{
    ...
    public function message_d($senderu)
    {
        if (Auth::check()) {
            // 从会话中检索用户信息
            $username = session('user_name');
            ...
            $messages_d = Message::where(function ($query) use ($senderu, $username) {
            ...
            // 渲染消息视图并返回HTML内容
            //$messagesHtml = view('auth.Message_d', compact('messages_d', 'username' , 'senderu'))->render();
            $htmlContent = null;
            foreach($messages_d as $message_user){
